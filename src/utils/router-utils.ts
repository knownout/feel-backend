...
interface ISchema<T extends Joi.AnySchema = Joi.AnySchema> {
  params?: T
  query?: T
  body?: T
}

export function routerUtils<Res extends Response, Req extends Request>(res: Res, req?: Req) {
  ...
  return {
    ...
    async errorBoundary<T extends (self: typeof this) => unknown>(exec: T) {
      ...
      try {
        ...
        console.log(error)
        if (error?.name === "Error") Logger.error(error)
        this.sendJSONRaw({
